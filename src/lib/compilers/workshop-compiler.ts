...
async function generateWorkshopTxt(pzpwConfig: PZPWConfig) {
  ...
  await writeFile("workshop/workshop.txt", content).catch(() =>
    console.error(chalk.red("Error while writing 'workshop/workshop.txt'")),
  );
}
...
export async function WorkshopCompiler(pzpwConfig: PZPWConfig, cachedir: string) {
  ...
  console.log(
    chalk.yellowBright(
      `- Copying preview image '${basename(previewImagePath)}' to 'workshop/${basename(previewImagePath)}'...`,
    ),
  );
  ...
  await copyDirRecursiveTo(outDir, "workshop/Contents/mods/");
