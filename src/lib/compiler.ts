...
export class Compiler {
  ...
  public async run() {
    ...
    await getPZPWConfig()
      .then(pzpwConfig => (this.pzpwConfig = pzpwConfig))
      .catch(() => {
        /** ignore */
      });
  ...
  private requirePZPWProject() {
    if (!this.pzpwConfig) throw chalk.red("This command must be executed from the root of your PZPW project.");
  }
  ...
  private async exec() {
    ...
    if (!command.name || command.name === "help") await this.printIntro();
    ...
    if (command.name === "help" && command.params.length > 0)
      await getCommandHelp(command.params[0] as string, true)
        .then(text => console.log(chalk.grey(text)))
        .catch(() => console.log(chalk.grey(`Command "${command.params[0] as string}" not found!`)));
    else if (command.name === "mods") await this.compileMods(command.params);
    else if (command.name === "workshop") await this.compileWorkshop(command.params);
    else if (command.name === "cachedir") await this.cachedirCommand(command.params);
    else if (command.name === "update") await this.updateCommand(command.params);
    else if (command.name === "clean") await this.cleanCommand(command.params);
    else if (command.name === "version") await this.versionCommand();
  ...
  private async compileWorkshop(params: (string | number)[]) {
    ...
    await this.compileMods(modIds).catch(error => {
      throw error;
    });
  ...
  private async cleanCommand(params: (string | number)[]) {
    ...
    if (!params[0]) return console.log(chalk.gray(await getCommandHelp("clean", true)));
    ...
    // Print debug
    if (this.args.debug) console.log(cachedir, pathToDelete);
